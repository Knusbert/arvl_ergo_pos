# -*- coding: utf-8 -*-
"""
Created on Wed Aug  9 07:53:56 2023

@author: bernhard.kloeter
"""
...
img_path = "data/ergo_static_01.png"
...
def prepare_model():
    ...
    model = torchvision.models.detection.keypointrcnn_resnet50_fpn(pretrained=True).to('cuda')
    # call the eval() method to prepare the model for inference mode.
    model.eval()


    # create the list of keypoints.
    keypoints = ['left_shoulder','right_shoulder','left_elbow',
    ...
    return model, transform, keypoints


def get_keypoints(img, model, transform):
...
def draw_keypoints_per_person(img, model, keypoint_threshold=2, conf_threshold=0.9):
    all_keypoints = model["keypoints"]
    all_scores = model["keypoints_scores"]
    confs = model["scores"]
...
def draw_skeleton_per_person(img, model, limbs, keypoint_threshold=2, conf_threshold=0.9):
    all_keypoints = model["keypoints"]
    all_scores = model["keypoints_scores"]
    confs = model["scores"]
          ...
          if confs[person_id]>conf_threshold:
            ...
            for limb_id in range(len(limbs)):
              ...
              if limb_score> keypoint_threshold:
                ...
                cv.line(img_copy, tuple(limb_loc1), tuple(limb_loc2), color, 10)
    return img_copy

def main():

    model, transform, keypoints  = prepare_model()
    output = get_keypoints(img, model, transform)
    ...
    st.write()
    keypoints_img = draw_keypoints_per_person(img, output,  keypoint_threshold=2)
    # cv2.imwrite("output/keypoints-img.jpg", keypoints_img)


    skeletal_img = draw_skeleton_per_person(img, output, limbs,keypoint_threshold=2)
    # cv2.imwrite("output/skeleton-img.jpg", skeletal_img)

    st.markdown("*Pose Detected in Image:*")
    st.image(img)
    st.image(keypoints_img)
    st.image(skeletal_img)
